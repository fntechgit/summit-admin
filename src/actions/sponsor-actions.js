...
export const SPONSOR_SPONSORSHIPS_ADDON_DELETED =
...
export const RESET_SPONSOR_EXTRA_QUESTION_FORM =
  "RESET_SPONSOR_EXTRA_QUESTION_FORM";
export const SPONSOR_EXTRA_QUESTION_VALUE_DELETED =
  "SPONSOR_EXTRA_QUESTION_VALUE_DELETED";
export const SPONSOR_EXTRA_QUESTION_VALUE_ADDED =
  "SPONSOR_EXTRA_QUESTION_VALUE_ADDED";
export const SPONSOR_EXTRA_QUESTION_VALUE_UPDATED =
...
export const SUMMIT_SPONSORSHIP_ORDER_UPDATED =
...
export const RECEIVE_SPONSORS_WITH_SCANS = "RECEIVE_SPONSORS_WITH_SCANS";
...
export const RESET_SPONSOR_ADVERTISEMENT_FORM =
  "RESET_SPONSOR_ADVERTISEMENT_FORM";
export const SPONSOR_ADVERTISEMENT_DELETED = "SPONSOR_ADVERTISEMENT_DELETED";
export const SPONSOR_ADVERTISEMENT_IMAGE_ATTACHED =
  "SPONSOR_ADVERTISEMENT_IMAGE_ATTACHED";
export const SPONSOR_ADVERTISEMENT_IMAGE_DELETED =
...
export const RECEIVE_SPONSOR_SOCIAL_NETWORKS =
...
export const RESET_SPONSOR_SOCIAL_NETWORK_FORM =
...
export const CLEAR_ALL_SELECTED_SPONSOR_PROMOCODES =
...
export const SET_SPONSOR_PROMOCODES_CURRENT_FLOW_EVENT =
  "SET_SPONSOR_PROMOCODES_CURRENT_FLOW_EVENT";
export const SET_SELECTED_ALL_SPONSOR_PROMOCODES =
...
export const getSponsorsWithBadgeScans = () => async (dispatch, getState) => {
  ...
  const _getSponsorsPerPage = (page) => {
    const params = {
      page,
      per_page: 50,
      expand: "company",
      fields: "id,company.name",
      relations: "none,company.none",
      access_token: accessToken,
    ...
    return getRequest(
      null,
      createAction(RECEIVE_SPONSORS_WITH_SCANS),
      ...
      if (current_page < last_page) {
        return _getSponsorsPerPage(current_page + 1);
      }
...
export const getSponsor = (sponsorId) => async (dispatch, getState) => {
  ...
  const params = {
    access_token: accessToken,
    expand:
      "company, members, sponsorship, sponsorship.type, featured_event, extra_questions, lead_report_setting",
...
export const saveAddonsToSponsorship =
  (addon, sponsorshipId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
    ...
    const normalizedAddons = normalizeAddons(addon);
    ...
    normalizedAddons.forEach((a) => {
      if (a.id) {
        promises.push(
          putRequest(
            null,
            createAction(SPONSOR_SPONSORSHIPS_ADDON_UPDATED),
            `${window.API_BASE_URL}/api/v1/summits/${currentSummit.id}/sponsors/${sponsorId}/sponsorships/${sponsorshipId}/add-ons/${a.id}`,
            a,
            snackbarErrorHandler
          )(params)(dispatch)
        );
        ...
        promises.push(
          postRequest(
            ...
            `${window.API_BASE_URL}/api/v1/summits/${currentSummit.id}/sponsors/${sponsorId}/sponsorships/${sponsorshipId}/add-ons`,
            a,
            snackbarErrorHandler
          )(params)(dispatch)
        );
      ...
      return Promise.all(promises)
        .then(() =>
          dispatch(
            snackbarSuccessHandler({
              title: T.translate("general.success"),
              html: T.translate("edit_sponsor.sponsorship_addons_saved")
            })
          )
        )
        .catch((err) => {
          console.log("Error updated addons: ", err);
          dispatch(
            snackbarSuccessHandler({
              title: T.translate("general.error"),
              html: T.translate(err)
            })
          );
        })
...
export const removeAddonToSponsorship =
  (addonId, sponsorshipId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
    ...
    return deleteRequest(
      null,
      createAction(SPONSOR_SPONSORSHIPS_ADDON_DELETED)({
        addonId,
        sponsorshipId
      }),
...
const normalizeAddons = (entity) => {
  const addonsArray = [...(entity.addons ?? [])];

  addonsArray.forEach((a) => {
    if (a.id) {
      delete a.created;
      delete a.last_edited;
    }
  });

  if (entity.newAddon) addonsArray.push(entity.newAddon);
...
export const addMemberToSponsor =
  (sponsorId, member) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const removeMemberFromSponsor =
  (sponsorId, memberId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const updateSponsorOrder =
  (sponsors, sponsorId, newOrder) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const createCompany = (company, callback) => async (dispatch) => {
  const accessToken = await getAccessTokenSafely();
...
export const getSponsorExtraQuestion =
  (extraQuestionId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
...
export const saveSponsorExtraQuestionValue =
  (questionId, entity) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
 ...
 * @returns {function(*=, *): *}
 */
export const updateSponsorExtraQuestionValueOrder =
...
export const deleteSponsorExtraQuestionValue =
  (questionId, valueId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
...
export const resetSummitSponsorshipForm = () => (dispatch) => {
...
export const removeSponsorshipBadgeImage =
  (sponsorshipId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const updateSummitSponsorhipOrder =
  (sponsorships, sponsorshipId, newOrder) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const deleteSummitSponsorship =
  (sponsorshipId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
const normalizeSponsorship = (entity) => {
  ...
  normalizedEntity.type_id = normalizedEntity.type
    ? normalizedEntity.type.id
    : 0;
...
const normalizeSocialNetwork = (entity) => {
  ...
  normalizedEntity.icon_css_class = entity.icon_css_class?.value
    ? entity.icon_css_class.value
    : entity.icon_css_class;
...
export const getBadgeScan = (scanId) => async (dispatch, getState) => {
  ...
  const params = {
    access_token: accessToken,
    expand:
      "scanned_by,badge,badge.ticket,badge.ticket.owner,badge.ticket.owner.member,sponsor,sponsor.extra_questions,sponsor.extra_questions.values,extra_questions",
    relations:
      "scanned_by,sponsor,sponsor.none,badge,scanned_by.none,badge.none,badge.ticket.none,badge.ticket.owner.none,badge.ticket.owner.member.none",
    fields:
      "id,notes,extra_questions,created,scan_date,attendee_first_name,attendee_last_name,attendee_email,attendee_company,sponsor.id,sponsor.extra_questions,scanned_by.email,scanned_by.first_name,scanned_by.last_name,badge.ticket,badge.ticket.owner,badge.ticket.owner.member.first_name,badge.ticket.owner.member.last_name,badge.ticket.owner.email,badge.ticket.owner.company,badge.ticket.owner.first_name,badge.ticket.owner.last_name"
  };
...
export const attachSponsorImage =
  (entity, file, picAttr) => async (dispatch) => {
    const accessToken = await getAccessTokenSafely();
...
export const removeSponsorImage = (entity, picAttr) => async (dispatch) => {
  dispatch(startLoading());
...
export const removeHeaderMobileImage =
  (entity) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    ...
    if (entity.id) {
      ...
      postRequest(
        ...
        dispatch(
          showMessage(success_message, () => {
            history.push(
              `/app/summits/${currentSummit.id}/sponsors/${currentSponsorId}/ads/${payload.response.id}`
            );
          })
        );
      });
...
export const getSponsorAdvertisement =
  (advertisementId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
...
export const updateSponsorAdsOrder =
  (ads, advertiseId, newOrder) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
...
export const resetSponsorAdvertisementForm = () => (dispatch) => {
  dispatch(createAction(RESET_SPONSOR_ADVERTISEMENT_FORM)({}));
};

export const deleteSponsorAdvertisement =
  (advertisementId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const submitSponsorAdvertisementImage =
  (entity, file) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const removeSponsorAdvertisementImage =
  (entity) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
  ...
  if (entity.id) {
    ...
    postRequest(
      ...
      dispatch(
        showMessage(success_message, () => {
          history.push(
            `/app/summits/${currentSummit.id}/sponsors/${currentSponsorId}/materials/${payload.response.id}`
          );
        })
      );
    });
...
export const getSponsorMaterial =
...
export const updateSponsorMaterialOrder =
  (materials, materialId, newOrder) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
...
export const resetSponsorMaterialForm = () => (dispatch) => {
  dispatch(createAction(RESET_SPONSOR_MATERIAL_FORM)({}));
};

export const deleteSponsorMaterial =
  (materialId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const getSponsorSocialNetworks =
  (sponsorId, page, perPage) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const saveSponsorSocialNetwork =
  (entity) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    ...
    if (entity.id) {
      ...
      postRequest(
        ...
        dispatch(
          showMessage(success_message, () => {
            history.push(
              `/app/summits/${currentSummit.id}/sponsors/${currentSponsorId}/social-networks/${payload.response.id}`
            );
          })
        );
      });
...
export const getSponsorSocialNetwork =
  (socialNetWorkId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    const {
...
export const resetSponsorSocialNetworkForm = () => (dispatch) => {
  dispatch(createAction(RESET_SPONSOR_SOCIAL_NETWORK_FORM)({}));
};

export const deleteSponsorSocialNetwork =
  (socialNetWorkId) => async (dispatch, getState) => {
    const { currentSummitState, currentSponsorState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const upsertSponsorLeadReportSettings =
  (sponsorId, allowed_columns) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const { currentSummit } = currentSummitState;
    const accessToken = await getAccessTokenSafely();

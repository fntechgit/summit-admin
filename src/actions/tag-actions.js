...
export const getTags =
  ...
  async (dispatch, getState) => {
    ...
    if (term) {
      ...
      filter.push(
        `tag=@${escapedTerm},tag@@${escapedTerm},tag==${escapedTerm}`
      );
    }
...
export const seedTagGroups = () => async (dispatch, getState) => {
  ...
  return postRequest(
    ...
    dispatch(
      showSuccessMessage(T.translate("edit_tag_group.tag_groups_seeded"))
    );
  });
};
...
export const copyAllTagsToCategory =
  (tagGroupId, categoryId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;

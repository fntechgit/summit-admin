...
export const getTicketTypes = (summit, term, order = 'name', orderDir = 1, currentPage = 1, perPage = 10, filters = {} ) => async (dispatch) => {
...
const parseTicketTypeFilters = (filters, term = null) => {
    ...
    if (filters.hasOwnProperty('badge_type_filter') && Array.isArray(filters.badge_type_filter)
        && filters.badge_type_filter.length > 0) {
        filter.push(`badge_type==${filters.badge_type_filter.join('||')}`);
    }

    if (filters.sale_period_filter && filters.sale_period_filter.some(e => e !== null)) {
        if(filters.sale_period_filter.every(e => e !== null )) {
            filter.push(`sales_start_date>=${filters.sale_period_filter[0]}`,`sales_end_date<=${filters.sale_period_filter[1]}`);
        } else {
            filter.push(`${filters.sale_period_filter[0] !== null && filters.sale_period_filter[0] !== 0 ? `sales_start_date>=${filters.sale_period_filter[0]}` : ``}${filters.sale_period_filter[1] !== null && filters.sale_period_filter[1] !== 0 ? `sales_end_date<=${filters.sale_period_filter[1]}` : ``}`);            
        }
    }

    if (term) {
        const escapedTerm = escapeFilterValue(term);
        let searchString = `name=@${escapedTerm},` +            

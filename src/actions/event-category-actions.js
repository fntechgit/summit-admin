...
export const RESET_EVENT_CATEGORY_GROUP_FORM =
...
export const RECEIVE_EVENT_CATEGORY_GROUP_META =
...
export const getEventCategory =
  (eventCategoryId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const updateEventCategoryOrder =
  (tracks, trackId, newOrder) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const saveEventCategory = (entity) => async (dispatch, getState) => {
  ...
  if (entity.id) {
    return putRequest(
      ...
      dispatch(stopLoading());
      dispatch(
        showSuccessMessage(T.translate("edit_event_category.category_saved"))
      );
    });
  }
...
export const copyEventCategories =
  (fromSummitId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const linkSubCategory =
  (trackId, subTrackId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const unlinkSubCategory =
  (trackId, subTrackId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const updateSubCategoryOrder =
  (trackId, subTrackId, order) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const saveEventCategoryGroup =
...
export const addCategoryToGroup =
  (groupId, category) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const removeCategoryFromGroup =
  (groupId, categoryId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const addAllowedGroupToGroup =
  (groupId, allowedGroup) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const removeAllowedGroupFromGroup =
  (groupId, allowedGroupId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;

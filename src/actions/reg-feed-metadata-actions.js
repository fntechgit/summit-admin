...
export const getRegFeedMetadataBySummit =
  (term = "", page = 1, perPage = 10, order = "id", orderDir = 0) =>
...
export const getRegFeedMetadata =
  (regFeedMetadataId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const saveRegFeedMetadata = (entity) => async (dispatch, getState) => {
  ...
  if (entity.id) {
    putRequest(
      ...
      dispatch(
        showSuccessMessage(
          T.translate("edit_reg_feed_metadata.reg_feed_metadata_saved")
        )
      );
    });
  } else {
    ...
    postRequest(
      ...
      dispatch(
        showMessage(success_message, () => {
          history.push(
            `/app/summits/${currentSummit.id}/reg-feed-metadata/${payload.response.id}`
          );
        })
      );
    });
...
export const deleteRegFeedMetadata =
  (regFeedMetadataId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;

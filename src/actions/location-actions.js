...
export const saveLocation =
  (entity, allClasses) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
      ...
      postRequest(
        ...
        dispatch(
          showMessage(success_message, () => {
            history.push(
              `/app/summits/${currentSummit.id}/locations/${payload.response.id}`
            );
          })
        );
      });
...
export const exportLocations = () => async (dispatch, getState) => {
  ...
  dispatch(
    getCSV(
      `${window.API_BASE_URL}/api/v1/summits/${currentSummit.id}/locations/csv`,
      params,
      filename
    )
  );
...
const normalizeEntity = (entity, allClasses) => {
  ...
  const locationClass = allClasses.find(
  ...
  normalizedEntity.opening_hour = normalizedEntity.opening_hour
    ? parseInt(normalizedEntity.opening_hour)
    : null;
  normalizedEntity.closing_hour = normalizedEntity.closing_hour
    ? parseInt(normalizedEntity.closing_hour)
    : null;
...
export const saveFloor =
  (locationId, entity, continueAdding) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
        ...
        dispatch(
          showMessage(success_message, () => {
            if (continueAdding) {
              ...
              history.push(
                `/app/summits/${currentSummit.id}/locations/${locationId}/floors/${payload.response.id}`
              );
            }
          })
        );
...
export const deleteFloor =
  (locationId, floorId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const attachFloorImage =
  (locationId, entity, file) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
const uploadFloorFile =
  (locationId, entity, file) => async (dispatch, getState) => {
    ...
    postRequest(
      ...
      dispatch(stopLoading());
      history.push(
        `/app/summits/${currentSummit.id}/locations/${locationId}/floors/${entity.id}`
      );
    });
  };

export const deleteFloorImage =
  (locationId, floorId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
        ...
        dispatch(
          showMessage(success_message, () => {
            if (continueAdding) {
              ...
              history.push(
                `/app/summits/${currentSummit.id}/locations/${locationId}/rooms/${payload.response.id}`
              );
            }
          })
        );
...
export const deleteRoom =
  (locationId, roomId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
const uploadRoomFile =
  (locationId, entity, file) => async (dispatch, getState) => {
    ...
    postRequest(
      ...
      dispatch(stopLoading());
      history.push(
        `/app/summits/${currentSummit.id}/locations/${locationId}/rooms/${entity.id}`
      );
    });
  };
...
export const removeAttributeFromRoom =
  (locationId, roomId, attributeId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
...
export const saveLocationImage =
  (locationId, entity, file) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;
    ...
    if (entity.id) {
      ...
      postFile(
        ...
        dispatch(
          showMessage(success_message, () => {
            history.push(
              `/app/summits/${currentSummit.id}/locations/${locationId}/images/${payload.response.id}`
            );
          })
        );
      });
    ...
    if (entity.id) {
      ...
      postFile(
        ...
        dispatch(
          showMessage(success_message, () => {
            history.push(
              `/app/summits/${currentSummit.id}/locations/${locationId}/maps/${payload.response.id}`
            );
          })
        );
      });
...
export const deleteLocationMap =
  (locationId, mapId) => async (dispatch, getState) => {
    const { currentSummitState } = getState();
    const accessToken = await getAccessTokenSafely();
    const { currentSummit } = currentSummitState;

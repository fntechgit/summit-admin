...
const UnlockScheduleButton = ({
  ...
  const createdBy =
    lock &&
    `${lock.created_by?.member?.first_name} ${lock.created_by?.member?.last_name}`;
  const createdDate = lock && epochToMoment(lock.created).format("M/D h:mm a");

  const lockCopy = lock ? (
    <p>
      This track has been locked by {createdBy} on {createdDate} UTC
    </p>
  ) : null;
  ...
  return (
    <>
      ...
      <Modal
        ...
        dialogClassName={styles.modalWrapper}
      >

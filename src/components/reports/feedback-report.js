...
class FeedbackReport extends React.Component {
  ...
  handleFilterChange(value) {
    this.setState({ groupBy: value.value }, () => {
      this.props.onReload();
    });
  }
      ...
      case "presentation":
        {
          processedData = data.map((it) => {
            let pres = forExport ? (
              ...
              <a
                onClick={() => {
                  history.push(`feedback_report/presentation/${it.id}`, {
                    name: it.title
                  });
                }}
              >
      ...
      case "track":
        {
          processedData = data.map((it) => {
            let track = forExport ? (
              ...
              <a
                onClick={() => {
                  history.push(`feedback_report/track/${it.id}`, {
                    name: it.title
                  });
                }}
              >
      ...
      case "speaker":
        {
          processedData = data.map((it) => {
            ...
            let speaker = forExport ? (
              speakerName
            ) : (
              <a
                onClick={() => {
                  history.push(`feedback_report/speaker/${it.id}`, {
                    name: speakerName
                  });
                }}
              >
  ...
  render() {
    let { data, extraData, totalCount, extraStat, sortKey, sortDir } =
      this.props;
    let { groupBy } = this.state;

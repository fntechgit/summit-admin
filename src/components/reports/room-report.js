...
class RoomReport extends React.Component {
  ...
  preProcessData(data, extraData, forExport = false) {
    ...
    let processedData = flatData.map((it) => {
      ...
      let capacity = forExport ? (
        it.location_venueroom_capacity
      ) : (
        <div className="text-center">{it.location_venueroom_capacity + ""}</div>
      );
      let speakers = forExport ? (
        it.speakerCount
      ) : (
        <div className="text-center">{it.speakerCount + ""}</div>
      );
      let head_count = forExport ? (
        it.headCount
      ) : (
        <div className="text-center">{it.headCount + ""}</div>
      );
      let scheduled = forExport ? (
        it.attendeeCount
      ) : (
        <div className="text-center">{it.attendeeCount + ""}</div>
      );
      let metrics = forExport ? (
        it.uniqueMetricCount
      ) : (
        <div className="text-center">{it.uniqueMetricCount + ""}</div>
      );
...
export default wrapReport(RoomReport, {
  pagination: false,
  filters: ["track", "room", "type"],
  grouped: true
});

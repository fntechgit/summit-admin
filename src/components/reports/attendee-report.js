...
class AttendeeReport extends React.Component {
  ...
  translateFilters = (reportQueryFilters) => {
    const { ticket_types, badge_features, questions, ...newFilters } =
      reportQueryFilters;
    ...
    if (questions) {
      ...
      const filter = Object.entries(parsedQuestions).map(
        ([questionId, answers]) => {
  ...
  render() {
    ...
    const { showQuestions } = this.state;
    const { attendee_extra_questions, ticket_types, badge_features } =
      currentSummit;

    if (!attendee_extra_questions || !data || name !== this.getName())
      return null;
    ...
    const parsedQuestions = attendee_extra_questions
      .filter((eq) => {
        ...
        if (
          allowed_badge_features_types.length > 0 &&
          featuresFilter.length > 0
        ) {
          allowedByFeature = allowed_badge_features_types.some((fid) =>
            featuresFilter.includes(fid.toString())
          );
        }

        if (allowed_ticket_types.length > 0 && ticketTypesFilter.length > 0) {
          allowedByTicketType = allowed_ticket_types.some((fid) =>
            ticketTypesFilter.includes(fid.toString())
          );
        }
      ...
      .map((eq) => ({
        ...eq,
        label_text: eq.label.replace(/<[^>]+>/g, "").slice(0, 60)
      }));
    ...
    const showQuestionsOptions = parsedQuestions.map((q) => ({
      label: q.label_text,
      value: q.id
    }));
        ...
        <div className="report-filters">
          <div className="row">
            <div className="col-md-4">
              <TicketTypeFilter
                ...
                types={ticket_types}
                onChange={(value) =>
                  onFilterChange("ticket_types", value, true)
                }
                isMulti
              />
            </div>
            <div className="col-md-4">
              <BadgeFeatureFilter
                ...
                onChange={(value) =>
                  onFilterChange("badge_features", value, true)
                }
                isMulti
              />
            </div>
            ...
            <div className="col-md-8">
              ...
              <Select
                ...
                placeholder={T.translate(
                  "reports.placeholders.select_questions"
                )}
          ...
          <div className="row">
            <div className="col-md-12">
              <button className="btn btn-primary pull-right" onClick={onReload}>
                {" "}
                Go{" "}
              </button>
            </div>

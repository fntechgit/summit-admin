    ...
    switch (question.type) {
      ...
      case "ComboBox":
        let value = answerValue
          ? questionValues.find((val) => val.id === parseInt(answerValue))
          : null;
        questionValues = questionValues.map((val) => ({
          ...val,
          value: val.id
        }));
        return (
      ...
      case "CheckBoxList":
        ...
        answerValue = answerValue
          ? answerValue.split(",").map((ansVal) => parseInt(ansVal))
          : [];
        return (
          <div>
            <label>
        ...
        {questions.map((q) => {
          ...
          return (
            <div className="row form-group" key={`question_answer_${q.id}`}>
              <div className="col-md-12">{this.getInput(q, answerValue)}</div>
            </div>
          );
        })}
      </div>

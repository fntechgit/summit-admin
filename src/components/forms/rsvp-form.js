  ...
  renderQuestion(q) {
    switch (q.class_name) {
      ...
      case "RSVPDropDownQuestionTemplate":
        let values = q.is_country_selector
          ? q.values.map((v) => ({ value: v.id, label: v.value }))
          : q.values;

        if (q.is_multiselect) {
          return (
            <Select placeholder={q.empty_string} options={values} isMulti />
          );
        } else {
          return <Select placeholder={q.empty_string} options={values} />;
        }
        break;
    }
  }

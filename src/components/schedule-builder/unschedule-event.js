...
const UnScheduleEvent = ({
  ...
  const title =
    event.title.slice(0, 75) + (event.title.length > 75 ? "..." : "");
  const rank = event.rank ? (
    <span className={`rank-status ${event.selection_status}`}>
      {" "}
      #{event.rank} - {event.selection_status}
    </span>
  ) : (
    <span />
  );
  ...
  const speakers =
    formatSpeakers(event.speakers).slice(0, 30) +
  ...
  return (
    <div
      className="row unschedule-event"
      ...
      <div className="row">
        <div className="col-md-12">
              ...
              >
                <span className="event-title">
                  {`${event.id} - ${title} ${
                    ...
                    event.duration ? ` - ${event.duration / 60} minutes` : ""
                  }`}
                  {rank}
                </span>
              </OverlayTrigger>
          ...
          <div className="event-actions">
            <i
              className="fa fa-pencil-square-o edit-unpublished-event-btn"
              title="edit event"
              aria-hidden="true"

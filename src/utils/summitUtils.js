...
export const flattenEventData = (e, summit) => {
    ...
    return {
        ...
        speakers: (e.speakers && e.speakers.length > 0) ? e.speakers.map(s => `${s.first_name} ${s.last_name}`).join(',') : 'N/A',
        ...
        sponsor: (e.sponsors.length > 0) ? e.sponsors.map(s => s.name).join(', ') : 'N/A',
        media_uploads: (e?.media_uploads?.length > 0) ? e?.media_uploads?.map( m => ({...m, created:moment(m.created * 1000).tz(summit.time_zone.name).format('MMMM Do YYYY, h:mm a') })) : 'N/A',
    };
}

  ...
  switch (type) {
    ...
    case RECEIVE_SPONSOR_PROMOCODES: {
      ...
      const promocodes = payload.response.data.map((p) => {
        const ticketTypes =
          p.allowed_ticket_types ||
          p.ticket_types_rules?.map((ttr) => ttr.ticket_type) ||
          [];
        return {
          ...p,
    ...
    case SELECT_SPONSOR_PROMOCODE: {
      ...
      if (selectedAll) {
        newState = {
          ...state,
          excludedIds: excludedIds.filter((it) => it !== promocodeId),
          selectedIds: []
        };
      } else {
        newState = {
          ...state,
          selectedIds: [...selectedIds, promocodeId],
          excludedIds: []
        };
      }
    ...
    case UNSELECT_SPONSOR_PROMOCODE: {
      ...
      if (selectedAll) {
        newState = {
          ...state,
          excludedIds: [...excludedIds, promocodeId],
          selectedIds: []
        };
      } else {
        newState = {
          ...state,
          selectedIds: selectedIds.filter((it) => it !== promocodeId),
          excludedIds: []
        };
      }
    ...
    case SET_SELECTED_ALL_SPONSOR_PROMOCODES: {
      ...
      const promocodes = state.promocodes.map((p) => ({
        ...p,
        checked: selectedAll
      }));
    ...
    case SEND_SPONSOR_PROMOCODES_EMAILS: {
      ...
      newState.promocodes = newState.promocodes.map((p) => ({
        ...p,
        checked: false
      }));

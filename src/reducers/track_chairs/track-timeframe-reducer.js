...
const trackTimeframeReducer = (state = DEFAULT_STATE, action) => {
  ...
  switch (type) {
    ...
    case LOCATION_TIMEFRAME_DELETED: {
      ...
      return {
        ...
        entity: {
          ...state.entity,
          proposed_schedule_allowed_locations:
            state.entity.proposed_schedule_allowed_locations.filter(
              (psal) => psal.id !== allowedLocationId
            )
        }
      };
    }
    ...
    case DAY_TIMEFRAME_ADDED: {
      const entity = { ...payload.response };
      const newLocs = state.entity.proposed_schedule_allowed_locations.map(
        (psal) => {
          if (psal.id === entity.allowed_location_id) {
            const newDayTimeframes = [
              ...psal.allowed_timeframes.filter((at) => at.id !== entity.id),
              entity
            ];
      ...
      return {
        ...
        entity: {
          ...state.entity,
          proposed_schedule_allowed_locations: newLocs
        }
      };
    }
      ...
      const newLocs = state.entity.proposed_schedule_allowed_locations.map(
        (psal) => {
          if (psal.id === allowedLocationId) {
            return {
              ...psal,
              allowed_timeframes: psal.allowed_timeframes.filter(
                (at) => at.id !== timeframeId
              )
            };
          }
          return psal;

...
const scheduleSettingsListReducer = (state = DEFAULT_STATE, action) => {
  ...
  switch (type) {
    ...
    case RECEIVE_ALL_SCHEDULE_SETTINGS: {
      ...
      return {
        ...state,
        scheduleSettings: [...scheduleSettings, ...state.scheduleSettings],
        totalScheduleSettings: total
      };
    }
    case SCHEDULE_SETTING_DELETED: {
      ...
      return {
        ...state,
        scheduleSettings: state.scheduleSettings.filter(
          (ss) => ss.id !== scheduleSettingId
        ),
        totalScheduleSettings: state.totalScheduleSettings - 1
      };
    }
    default:
      return state;

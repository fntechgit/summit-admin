...
const signageReducer = (state = DEFAULT_STATE, action) => {
  ...
  switch (type) {
    ...
    case RECEIVE_SIGNAGE_EVENTS: {
      const { current_page, total, last_page, data, summit_tz } =
        payload.response;
      const events = data.map((ev) => {
        const start_date_str = epochToMomentTimeZone(
          ev.start_date,
          state.summitTz
        ).format("M/D h:mm a");
        const end_date_str = epochToMomentTimeZone(
          ev.end_date,
          state.summitTz
        ).format("M/D h:mm a");
        ...
        return {
          id: ev.id,
          title: ev.title,
          speakers_str: ev.speakers
            ? ev.speakers
                .map((sp) => `${sp.first_name} ${sp.last_name}`)
                .join(", ")
            : "N/A",
          floor_loc: ev.location.floor
            ? `${ev.location.floor?.name} / ${ev.location.name}`
            : ev.location.name,
          start_date_str,
          end_date_str,
    ...
    case RECEIVE_SIGNAGE_BANNERS: {
      ...
      const staticBanner = data.find(
        (ban) => ban.class_name === "SummitLocationBanner"
      ) || { content: "" };
      const rawBanners = data.filter(
        (ban) => ban.class_name === "ScheduledSummitLocationBanner"
      );
      ...
      return {
        ...
        banners,
        staticBanner,
        totalEntries: rawBanners.length,
        currentPage: current_page,
        lastPage: last_page
      };
    }
    ...
    case SIGNAGE_BANNER_DELETED: {
      ...
      return {
        ...state,
        banners: state.banners.filter((ban) => ban.id !== bannerId)
      };
    }
...
const mapBanner = (ban, summitTz) => {
  const start_date_str = epochToMomentTimeZone(ban.start_date, summitTz).format(
    "M/D h:mm a"
  );
  const end_date_str = epochToMomentTimeZone(ban.end_date, summitTz).format(
    "M/D h:mm a"
  );
  ...
  return {
    ...
    content: ban.content,
    type: ban.type,
    floor_loc: ban.location.floor
      ? `${ban.location.floor?.name} / ${ban.location.name}`
      : ban.location.name,

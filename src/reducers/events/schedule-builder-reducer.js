...
const scheduleBuilderReducer = (state = DEFAULT_STATE, action) => {
  ...
  switch (type) {
    ...
    case CHANGE_CURRENT_PRESENTATION_SELECTION_STATUS: {
      ...
      return {
        ...state,
        currentPresentationSelectionStatus: presentationSelectionStatus
      };
    }
    case CHANGE_CURRENT_PRESENTATION_SELECTION_PLAN: {
      ...
      return {
        ...state,
        currentPresentationSelectionPlan: presentationSelectionPlan
      };
    }
    ...
    case CLEAR_PROPOSED_EVENTS: {
      const { proposedSchedDay, proposedSchedLocation, proposedSchedTrack } =
        payload;
      return {
        ...state,
        proposedSchedEvents: [],
    ...
    case CHANGE_CURRENT_SCHEDULE_SEARCH_TERM: {
      let { term } = payload;
      let scheduleEventsSearch =
        term == null || term === "" ? [] : state.scheduleEventsSearch;
      return {
        ...state,
        scheduleEventsSearch,
        scheduleEventsCurrentSearchTerm: term
      };
    }
    case RECEIVE_SCHEDULE_EVENTS_SEARCH_PAGE: {
      let { data } = payload.response;
      return { ...state, scheduleEventsSearch: data };
    }
    case RECEIVE_SCHEDULE_EVENTS_PAGE: {
      let { data } = payload.response;
      return { ...state, scheduleEvents: data };
    }
    ...
    case RECEIVE_PROPOSED_SCHEDULE: {
      ...
      const proposedSchedEvents = data.map((ev) => {
        ...
        return {
          ...
          description: summit_event.description,
          published: summit_event.published
        };
      });
    ...
    case RECEIVE_SHOW_ALWAYS_EVENTS: {
      ...
      return {
        ...state,
        proposedSchedEvents: [...proposedSchedEvents, ...showAlwaysEvents]
      };
    }
    case PROPOSED_EVENTS_PUBLISHED: {
      return {
        ...state,
        currentLocation: state.proposedSchedLocation,
        currentDay: state.proposedSchedDay
      };
    }
    case RECEIVE_PROPOSED_SCHED_LOCKS: {
      const locks = payload.response.data;
      const proposedSchedLock = locks.find(
        (lock) => lock.track_id === state.proposedSchedTrack?.id
      );
    ...
    case UNPUBLISHED_EVENT: {
      ...
      let scheduleEvents = state.scheduleEvents.filter(
    ...
    case REQUEST_PUBLISH_EVENT:
      const { currentSummit, currentLocation, event, startTime, day, minutes } =
        payload;
      let eventModel = new SummitEvent(event, currentSummit);
      let [eventStarDateTime, eventEndDateTime] = eventModel.calculateNewDates(
        day,
        startTime,
        minutes
      );
      ...
      let unScheduleEvents = state.unScheduleEvents.filter(
        (item) => event.id !== item.id
      );
    ...
    case ERROR_PUBLISH_EVENT:
      ...
      return { ...state, scheduleEvents: [...state.scheduleEvents] };

...
const eventListReducer = (state = DEFAULT_STATE, action) => {
    ...
    switch (type) {
        ...
        case RECEIVE_EVENTS: {
            ...
            // let displayEvents = payload.response.data.map(e => {
            ...
            //         speakers: (e.speakers && e.speakers.length > 0) ? e.speakers : 'N/A',
        ...
        //     let { bulkEvents } = payload;
        //     eventBulkList = [...eventBulkList, ...bulkEvents];
        //     return {...state, eventBulkList};
        // }
        case UPDATE_EVENT_DATA_BULK: {
            let { eventId, mutator } = payload;
            let { events } = state;
            bulkEvents = bulkEvents.map(event => {
                return event.id === eventId ?  mutator(event) : event
            });
            return { ... state, bulkEvents }
        }

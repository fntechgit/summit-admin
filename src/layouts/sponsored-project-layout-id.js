...
class SponsoredProjectLayoutId extends React.Component {
  ...
  render() {
    ...
    let breadcrumb = currentProject.id
      ? currentProject.name
    ...
    return (
      <div>
        ...
        <Switch>
          <Route
            path={`${match.url}/sponsorship-types`}
            render={(props) => (
              <div>
                <Breadcrumb
                  data={{
                    title: T.translate(
                      "edit_sponsored_project.sponsorship_types"
                    ),
                    pathname: match.url
                  }}
                />
                <Switch>
                  <Route
                    path={`${props.match.url}/:sponsorship_id(\\d+)`}
                    component={SponsoredProjectSponsorshipTypeLayout}
                  />
                  <Route
                    exact
                    strict
          ...
          <Route
            ...
            exact
            path={match.url}
            component={EditSponsoredProjectPage}
          />
        </Switch>
      </div>
    );

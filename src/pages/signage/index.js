...
const SignagePage = ({
  ...
  const [staticBannerLoc, setStaticBannerLoc] = useState(
    staticBanner?.content || ""
  );
  const templateOptions =
    templates?.map((tmp) => ({ value: tmp.file, label: tmp.name })) || [];
  ...
  const viewSign = () => {
    window.open(
      `${window.SIGNAGE_BASE_URL}/${sign?.template}#/?summit=${summit.id}&location=${locationId}`
    );
  };
  ...
  const eventsColumns = [
    ...
    {
      columnKey: "start_date_str",
      value: T.translate("signage.start_date"),
      sortable: true
    },
  ...
  return (
    <div className="signage-wrapper">
      <Breadcrumb
        data={{ title: T.translate("signage.signs"), pathname: match.url }}
      />

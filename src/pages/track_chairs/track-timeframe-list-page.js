...
const TrackTimeframeListPage = ({
  ...
  const handleEdit = (track_id) => {
    history.push(
      `/app/summits/${currentSummit.id}/track-chairs/track-timeframes/${track_id}`
    );
  };
  ...
  const handleNew = () => {
    history.push(
      `/app/summits/${currentSummit.id}/track-chairs/track-timeframes/new`
    );
  };
  ...
  const columns = [
    ...
    {
      columnKey: "locationsStr",
      value: T.translate("track_timeframes.locations")
    }
  ];

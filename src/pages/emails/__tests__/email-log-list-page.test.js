...
describe("SentEmailListPage", () => {
  ...
  describe("pagination", () => {
    ...
    beforeEach(() => {
      requestedMeta.length = 0;
      getRequest.mockImplementation(
        (
            requestActionCreator,
            _receiveActionCreator,
            _url,
            _errorHandler,
            meta
          ) =>
          () =>
          (dispatch) => {
            requestedMeta.push(meta);
            dispatch(requestActionCreator(meta));
    ...
    it("keeps sending the updated page size after a page change", async () => {
      const store = buildStore({ emails: [{ id: 1 }] });
      ...
      await act(async () => {
        await userEvent.click(
          screen.getByRole("button", { name: "set-per-page-50" })
        );
      });
      await act(async () => {
        await userEvent.click(
      ...
      expect(store.getState().emailLogListState.perPage).toBe(50);
      expect(requestedMeta.at(-1).perPage).toBe(50);
      expect(requestedMeta.at(-1).page).toBe(2);
    });
  });
});

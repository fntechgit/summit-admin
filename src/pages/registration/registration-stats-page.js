...
const RegistrationStatsPage = ({ currentSummit, match, loading, ...props }) => {
  ...
  const timeOptions = ["Minute", "Hour", "Day"].map((op) => ({
    value: op,
    label: op
  }));
  ...
  const totalTickets =
    props.total_active_tickets + props.total_inactive_tickets;

  const totalTicketsSold = props.total_active_tickets;

  const totalTicketsAvailable = props.total_tickets_per_type.reduce(
    (res, it) => res + parseInt(it.available_qty),
    0
  );

  const hasInfiniteTicketsAvailable = props.total_tickets_per_type.some(
    (it) => it.available_qty === 0
  );

  const totalTicketsCheckedIn = props.total_tickets_per_type.reduce(
    (res, it) => res + parseInt(it.checkin_qty),
    0
  );

  const sortedTicketTypes = props.total_tickets_per_type.sort(
    (a, b) => b.sold_qty - a.sold_qty
  );

  const sortedTicketPerBadgeTypes = props.total_badges_per_type.sort(
    (a, b) => b.badges_qty - a.badges_qty
  );

  const totalTicketsWBadgeType = props.total_badges_per_type.reduce(
    (res, it) => res + parseInt(it.badges_qty),
    0
  );

  const totalTicketsSoldWBadgeFeature =
    props.total_tickets_per_badge_feature.reduce(
      (res, it) => res + parseInt(it.sold_qty),
      0
    );

  const totalTicketsCheckedInWBadgeFeature =
    props.total_tickets_per_badge_feature.reduce(
      (res, it) => res + parseInt(it.checkin_qty),
      0
    );

  const sortedTicketsPerBadgeFeature =
    props.total_tickets_per_badge_feature.sort(
      (a, b) => b.sold_qty - a.sold_qty
    );

  const totalRealAttendees =
    props.total_checked_in_attendees + props.total_non_checked_in_attendees;

  const totalVirtualAttendees =
    props.total_virtual_attendees +
    props.total_virtual_non_checked_in_attendees;
      ...
      <div>
        <div className="row">
          <div className="col-md-4">
            ...
            &nbsp;{T.translate("dashboard.payment_amount_collected")}&nbsp;
          ...
          <div className="col-md-4">
            <i className="fa fa-money" />
            &nbsp;{T.translate("dashboard.payment_net_amount_collected")}&nbsp;
            <strong>
              {toPrice(
                parseFloat(props.total_payment_amount_collected) -
                  parseFloat(props.total_refund_amount_emitted)
              )}
            </strong>
          </div>
        </div>
        ...
        {totalTickets > 0 && (
          <PieGraph
            ...
            subtitle={[
              `Total orders: ${props.total_orders}`,
        ...
        {(totalTicketsAvailable > 0 || hasInfiniteTicketsAvailable) && (
          <PieGraph
            ...
            subtitle={[
              ...
              `Tickets available: ${
                hasInfiniteTicketsAvailable ? "∞" : totalTicketsAvailable
              }`
            ]}
            ...
            data={sortedTicketTypes.map((tt) => ({
              ...
              label: `${trimString(tt.type)} : ${tt.sold_qty} / ${
                tt.available_qty || "∞"
              }`
            }))}
        ...
        {totalTicketsSold > 0 && (
          <PieGraph
            ...
            subtitle={[
              `Tickets w/ Badge type: ${totalTicketsWBadgeType}`,
              `Tickets active: ${totalTicketsSold}`
            ]}
        ...
        {totalTicketsSold > 0 && (
          <PieGraph
            ...
            subtitle={[
              `Tickets sold with badge features applied: ${totalTicketsSoldWBadgeFeature}`,
              `Tickets sold: ${totalTicketsSold}`
            ]}
        ...
        {totalAttendees > 0 && (
          <>
            <PieGraph
              ...
              subtitle={[
                `Tickets checked in: ${totalTicketsCheckedIn}`,
                `Tickets sold: ${totalTicketsSold}`
              ]}
              ...
              data={sortedTicketTypes.map((tt) => ({
                ...
                label: `${trimString(tt.type)} : ${tt.checkin_qty} / ${
                  tt.sold_qty
                }`
              }))}
              labels={sortedTicketTypes.map((tt) => {
            ...
            <PieGraph
              ...
              subtitle={[
                `Tickets checked in with badge features applied: ${totalTicketsCheckedInWBadgeFeature}`,
                `Tickets sold: ${totalTicketsSold}`
              ]}
          ...
          <PieGraph
            title={
              <>
                ...
                &nbsp;
                {T.translate("dashboard.in_person_attendees")} (
                {props.total_checked_in_attendees +
                  props.total_non_checked_in_attendees}
                )
              </>
            }
            data={[
              {
                value: props.total_checked_in_attendees,
                total: totalRealAttendees
              },
              {
                value: props.total_non_checked_in_attendees,
                total: totalRealAttendees
              }
            ]}
            labels={[
          ...
          <PieGraph
            title={
              <>
                <i className="fa fa-users" />
                &nbsp;
                {T.translate("dashboard.virtual_attendees")} (
                {props.total_virtual_attendees +
                  props.total_virtual_non_checked_in_attendees}
                )
              </>
            ...
            data={[
              {
                value: props.total_virtual_attendees,
                total: totalVirtualAttendees
              },
              {
                value: props.total_virtual_non_checked_in_attendees,
                total: totalVirtualAttendees
              }
            ]}
            labels={[
            ...
            colors={["rgba(255, 159, 64, 1)", "rgba(255, 99, 132, 1)"]}
          />
        )}

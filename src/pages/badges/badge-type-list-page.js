...
class BadgeTypeListPage extends React.Component {
  ...
  handleEdit(badge_type_id) {
    ...
    history.push(
      `/app/summits/${currentSummit.id}/badge-types/${badge_type_id}`
    );
  }
  ...
  handleDelete(badgeTypeId) {
    ...
    Swal.fire({
      ...
      text:
        T.translate("badge_type_list.remove_warning") + " " + badgeType.name,
      type: "warning",
  ...
  render() {
    let { currentSummit, badgeTypes, order, orderDir, totalBadgeTypes } =
      this.props;

    badgeTypes = badgeTypes.map((bt) => {
      return { ...bt, is_default: bt.is_default ? "Yes" : "No" };
    });

    const columns = [
      {
        columnKey: "name",
        value: T.translate("badge_type_list.name"),
        sortable: true
      },
      {
        columnKey: "is_default",
        value: T.translate("badge_type_list.is_default")
      },
      {
        columnKey: "description",
        value: T.translate("badge_type_list.description")
      },
      {
        columnKey: "access_level_names",
        value: T.translate("badge_type_list.access_levels")
      }
    ];
    ...
    return (
      <div className="container">
        <h3>
          {" "}
          {T.translate("badge_type_list.badge_type_list")} ({totalBadgeTypes})
        </h3>
        <div className={"row"}>
          <div className="col-md-6 text-right col-md-offset-6">
            <button
              className="btn btn-primary right-space"
              onClick={this.handleNewBadgeType}
            >
